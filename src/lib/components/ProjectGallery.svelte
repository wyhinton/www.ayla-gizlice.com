<script lang="ts">
  ...
  function getImageUrl(project: Project, imageNumber: number): string {
  ...
  function hasImages(project: Project): boolean {
    return !!(
      getImageUrl(project, 1) ||
      getImageUrl(project, 2) ||
      getImageUrl(project, 3)
    );
  }
...
>
  <div class="sectionElementContainer" id="sectionElementContainer_{index}">
    <div
      class="descriptionPaddingContainer"
      id="descriptionPaddingContainer_{index}"
    >
      <div class="descriptionDiv" id="descriptionDiv_{index}">
        ...
        <p class="projInfo" id="projInfo_{index}">
          ...
          {#if project.Collaborator && project.Collaborator !== "empty"}
            ...
            <a
              href={project.Collaborator_Link}
              target="_blank"
    ...
    {#if hasImages(project) || hasVideo(project)}
      <div class="img_div" id="img_div{index}">
        {#if getImageUrl(project, 1)}
          <div
            class="imageContainerPadding"
            id="imageContainerPadding1_{index}"
          >
            <div class="singleImageContainer" id="singleImageContainer_{index}">
              <a
                class="lightBoxLink"
                href={getImageUrl(project, 1)}
                target="_blank"
                rel="noopener noreferrer"
              >
                <img
        ...
        {#if getImageUrl(project, 2)}
          <div
            class="imageContainerPadding"
            id="imageContainerPadding2_{index}"
          >
            <div class="singleImageContainer">
              <a
                class="lightBoxLink"
                href={getImageUrl(project, 2)}
                target="_blank"
                rel="noopener noreferrer"
              >
                <img
        ...
        {#if getImageUrl(project, 3)}
          <div
            class="imageContainerPadding"
            id="imageContainerPadding3_{index}"
          >
            <div class="singleImageContainer">
              <a
                class="lightBoxLink"
                href={getImageUrl(project, 3)}
                target="_blank"
                rel="noopener noreferrer"
              >
                <img
