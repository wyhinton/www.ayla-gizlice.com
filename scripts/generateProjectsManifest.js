...
async function generateManifest() {
  try {
    ...
    // Extract unique categories in order of appearance (not sorted alphabetically)
...
export const projectsManifest: Project[] = ${JSON.stringify(
      processedProjects,
      null,
      2
    )};

export const uniqueCategories: string[] = ${JSON.stringify(
      categoriesInOrder,
      null,
      2
    )};
`;
    ...
    console.log(
      `📊 Generated manifest for ${processedProjects.length} projects with ${categoriesInOrder.length} categories`
    );
  } catch (error) {
...
if (!SHEET_URL) {
  ...
  console.warn(
    "📝 Creating placeholder manifest. Set VITE_GOOGLE_SHEET_ID to fetch from Google Sheets"
  );
